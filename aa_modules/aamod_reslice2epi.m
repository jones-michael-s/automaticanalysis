...
%
function [aap, resp] = aamod_reslice2epi(aap, task, varargin)
...
switch task
    ...
    case 'doit'
        ...
        % try subject leve, if not, look in first session (e.g. for meanepi)
...
% 		end
		...
		% since the reference stream is epi, there must be a session, so
		% there must be a first session. Just look there from the get-go:
		% (the try-catch is generating an aa-failed error message!)
