...
switch task
    ...
    case 'doit'
		...
		tsdiffana = load(temp);
		% I think this is the same as DVARS (albeit scaled different)
		SV = tsdiffana.qa.global.diff / mean(tsdiffana.qa.global.mean);
		GLOBALMEAN = tsdiffana.qa.global.mean;
		...
		if (strcmp(aap.options.wheretoprocess, 'localsingle'))
			figh = figure('Color',[1 1 1],'Position',[0 0 800 960],'Visible', 'off');
			movegui(figh,'center');
			set(figh,'Visible', 'on');
		else
			figh = figure('Color',[1 1 1],'Position',[0 0 800 960]);
		end
