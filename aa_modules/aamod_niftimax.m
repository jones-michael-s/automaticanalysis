function [ aap,resp ] = aamod_niftimax(aap, task, subj)

% extract the whole-brain maximum voxel value in a nifti file, and, optionally,
...
switch task
    ...
    case 'doit'
		...
		for index = 1:size(nifti_list,1)
			[ this_max, this_weighted_max ] = nifti_wholebrain_max(nifti_list(index,:), aap.tasklist.currenttask.settings.weight);
		...
		if (~isempty(aap.tasklist.currenttask.settings.TPMdir))
			...
			for index = 1:size(nifti_list,1)
				[ this_max, this_weighted_max ] = nifti_max_from_TPM(nifti_list(index,:), TPM_list);
		...
		if (~isempty(aap.tasklist.currenttask.settings.SEEDS))
			...
			for index = 1:size(nifti_list,1)
				[ this_max, this_weighted_max ] = nifti_max_from_seeds(aap, nifti_list(index,:), SEED_list.seed);			
...
%---------------------------------------------------------------------------------
function [ vmax, weighted_vmax ] = nifti_wholebrain_max(fname, weight)
%---------------------------------------------------------------------------------
...
%---------------------------------------------------------------------------------
function [ vmax, weighted_vmax ] = nifti_max_from_TPM(fname, TPMlist)
%---------------------------------------------------------------------------------
...
%---------------------------------------------------------------------------------
function [ vmax, weighted_vmax ] = nifti_max_from_seeds(aap, fname, SEEDS)
%---------------------------------------------------------------------------------
